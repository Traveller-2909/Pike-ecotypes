...
Saldata <- Saldata%>%filter(identifier=="Spring_2021")%>%
  group_by(year, month, id, waterbody)%>%
  summarise(name=name,
            id=id,
            waterbody=waterbody,
            month=month,
...
Saldata[Saldata$identifier=="20222"&Saldata$waterbody=="SB",]$Long <- 13.15835
...
for (i in 1:length(monthID)) {
    ...
    ipdw <- setNames(ipdw[[1]], "Spring_2021")
    ...
    sras2 <- stack(sras2, ipdw)
}

plot(sras$Spring_2021)
...
smean <- calc(sras, fun = mean)
ssd <- calc(sras, fun = sd)
srange <- calc(sras, fun = range)
smax <- calc(sras, fun = max)

srange_absolute <- srange$layer.2-srange$layer.1
...
writeRaster(smean, "Data/Spatial/Output/Sal-ipdw_mean_07-22", format="GTiff",
            overwrite=T)
writeRaster(ssd, "Data/Spatial/Output/Sal-ipdw_sd_07-22", format="GTiff", overwrite=T)
writeRaster(srange_absolute,"Data/Spatial/Output/Sal-ipdw_range_07-22", format="GTiff", overwrite=T)
