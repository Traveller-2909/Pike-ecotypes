...
Bclust_lowres <- Bclust_lowres%>%filter(ID!="BH-01908")%>%filter(age>=2, year<3)
Fclust_lowres <- Fclust_lowres%>%filter(age>=2, year<3)
...
plot(Oclust_B$k_3, type="centroid", linetype="solid", cex=1)+
...
ggplot(fishassign_F, aes(capt, fill=clust_O))+
  ...
  scale_fill_manual(breaks=c("Low FW|cold temp",
                             "Int FW|const temp",
                             "Anadromous|shift temp"), 
                    values = mypallette)+
...
write.table(fishassign_B, file = "Data/otoliths/Year_2_clusters.txt", row.names = F,
            dec = ".", sep = "\t")
write.table(fishassign_F, file = "Data/otoliths/Year_2_clusters.txt", row.names = F,
            dec = ".", sep = "\t")
