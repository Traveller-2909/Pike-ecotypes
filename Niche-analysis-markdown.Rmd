...
LUNG_sal_temp_rivers2 <- LUNG_sal_temp_rivers%>%
...
LUNG_sal_temp_coast2 <- LUNG_sal_temp_coast%>%
  pivot_longer(cols = c(11:22), names_to = "month",
               values_to = "value", values_drop_na = F)

# convert to wide
LUNG_sal_temp_coast3 <- spread(LUNG_sal_temp_coast2, parameter, value)
head(LUNG_sal_temp_coast)
LUNG_sal_temp_rivers3 <- spread(LUNG_sal_temp_rivers2, parameter, value)
head(LUNG_sal_temp_rivers)
LUNG_sal_temp_rivers3$SAL <- 0

LUNGdata_all <- rbind(LUNG_sal_temp_coast3, LUNG_sal_temp_rivers3)
names(LUNGdata_all)[12] <- paste("WT")
...
LUNGdata_monthsw <- LUNGdata_months%>%pivot_wider(names_from = month, values_from = c(SAL, W.T, turbidity))
...
LUNGdata_complete <- LUNGdata_months%>%
  ...
  ungroup()%>%
  group_by(id, month)%>%
  mutate(WT=case_when(is.na(WT)~mean(WT, na.rm = T),
                            TRUE ~ WT))
...
```{r clean up ODIN data, include=FALSE, eval=FALSE}
rm(list = ls())
ODIN2 <- fread("Data/ODIN2/odin2_2023-04-24_105959_reprojected.csv")

Odin3 <- ODIN2%>%
  transmute(id=`Station name`,
            year=Year,
            month=Month,
            day=Day,
            Long=as.numeric(`Longitude(dec)`),
            Lat=as.numeric(`Latitude(dec)`),
...
fwrite(Odin4, file = "Data/Odindata_complete.csv")
```

```{r Rene´s files, include=FALSE, eval=FALSE}
rm(list = ls())
Renedata1 <- nc_open("Data/Rene_Friedland/KHJ.nc")

{
    sink("Data/Rene_Friedland/KHJ_meta.txt")
 print(Renedata1)
    sink()
}

Long <- ncvar_get(Renedata1, "lon")
Lat <- ncvar_get(Renedata1, "lat", verbose = F)
time <- ncvar_get(Renedata1, "time")

Sdate <- as.Date(time, origin = "0000-01-01")

SAL <- ncvar_get(Renedata1, "Salt")
WT <- ncvar_get(Renedata1, "Temp")

Station1 <- data.frame(name="KHJ", 
                       id="0313040702", 
                       waterbody="Stettiner Haff", 
                       month=month(Sdate), 
                       year=year(Sdate), 
                       identifier=paste0(month(Sdate), "_", year(Sdate)),
                       Lat=Lat,
                       Long=Long,
                       type="Coast",
                       turbidity=NA,
...
Renedata2 <- nc_open("Data/Rene_Friedland/KHM.nc")

{
    sink("Data/Rene_Friedland/KHM_meta.txt")
 print(Renedata1)
    sink()
}

Long <- ncvar_get(Renedata2, "lon")
Lat <- ncvar_get(Renedata2, "lat", verbose = F)
time <- ncvar_get(Renedata2, "time")

Sdate <- as.Date(time, origin = "0000-01-01")

SAL <- ncvar_get(Renedata2, "Salt")
WT <- ncvar_get(Renedata2, "Temp")

Station2 <- data.frame(name="KHM", 
                       id="0313040905", 
                       waterbody="Stettiner Haff", 
                       month=month(Sdate), 
                       year=year(Sdate), 
...
Turb1 <- fread("Data/Rene_Friedland/Sichttiefe_KHJ.CSV")
Turb1$name <- "KHJ"
Turb2 <- fread("Data/Rene_Friedland/Sichttiefe_KHM.CSV")
Turb2$name <- "KHM"

Turbturb <- rbind(Turb1, Turb2)

Turbturb$identifier <- paste0(month(parse_date_time(Turbturb$DATUM_Uhrzeit, "dmy")), "_",year(parse_date_time(Turbturb$DATUM_Uhrzeit, "dmy")))

Stettiner <- rbind(Station1, Station2)

Stettiner <- Stettiner%>%
  left_join(Turbturb, by = c("name","identifier"))%>%
  transmute(name=name,
            id=id, 
            waterbody=waterbody, 
            month=month, 
...
fwrite(Stettiner, "Data/Stettiner_Haff_complete.csv")
```
...
#Translocation data
Felidata <- read.delim(file = "Data/Translocations.txt", sep = ",", header = T, stringsAsFactors = F)

#Baltic data
Odindata <- fread("Data/Odindata_complete.csv")

#Stettiner Haff
Stettiner <- fread("Data/Stettiner_Haff_complete.csv")
...
LUNGdata <- LUNGdata%>%
  transmute(name = name,
            ...
            month = month,
            year = year,
            identifier = case_when(year==2021&month==3|year==2021&month==4~"Spring_2021",
                                   TRUE~paste0(month, "_", year)),
            Lat = Lat,
            Long = Long,
            type = type,
            turbidity=turbidity,
...
#Transloc fish
Felidata <- Felidata%>%
  transmute(name="Translocation",
            id="trans",
            waterbody=paste0(Floy,Origin),
            month=lubridate::month(parse_date_time(Date.and.Time, "ymd HM")),
            year=lubridate::year(parse_date_time(Date.and.Time, "ymd HM")),
            identifier=paste0("Spring_",year),
            Lat=Latitude,
            Long=Longitude,
            type="trans",
            SAL=Salinity,
...
#Filter for interesting tributaries
Tributaries <- LUNGdata%>%
  filter(waterbody=="Sehrowbach"|waterbody=="Barthe"|waterbody=="Ziese"|waterbody=="Duwenbeek"|waterbody=="Ryck")
Lagoons <- LUNGdata%>%
  filter(type=="Coast")

Alldata <- rbind(Lagoons, Stettiner)
#Join & filter out freshwater, filter for years/months
#Saldata <- rbind(Tributaries, Lagoons, Samplingdata, Loggerdata, Felidata)
Saldata <- Alldata

Saldata <- Saldata%>%
  filter(year > 2018 & turbidity <=5)%>%
  group_by(year, name, waterbody, )%>%
  summarise(name=name,
            waterbody=waterbody,
            year=year,
            identifier=as.character(year),
            Lat=as.numeric(Lat),
...
#load in land layer
Land <- readOGR("Data/Spatial/Studyarea_buffered.gpkg")
Land2 <- readOGR("Data/Spatial/Baltic_landmass.gpkg") #We need a Baltic shape!
...
for (i in 1:length(monthID)) {
    ...
    fulldataset.over <- over(pnts, gridpol)
    fulldataset.over <- cbind(data.frame(fulldataset.over), 
                          setNames(data.frame(pnts), c("year", "name",
                                                       "waterbody",
                                                       "identifier",
                                                       "Lat", "Long", "type",
                                                       "turbidity", "SAL", "WT",
...
mean_turb <- calc(raster, fun = mean)
...
writeRaster(mean_turb, "Data/Spatial/Output/Turbidity_raster_book_revised", format="GTiff", overwrite=T)
...
Bclust_lowres <- Bclust_lowres%>%filter(ID!="BH-01908")
Fclust_lowres <- Fclust_lowres
...
dataB1 <- interpol(series = Bclust_lowres, length = 35, element = "Sr")
dataB2 <- interpol(series = Bclust_lowres, length = 35, element = "d18O_res")

dataF1 <- interpol(series = Fclust_lowres, length = 40, element = "Sr")
dataF2 <- interpol(series = Fclust_lowres, length = 40, element = "d18O_res")
...
names(Oclust_B) <- paste0("k_",2L:6L)
...
names(Oclust_F) <- paste0("k_",2L:6L)
...
plot(Oclust_B$k_3, type="centroid", linetype="solid", 
     alpha=0.5, cex=1)+
...
plot(Oclust_B$k_3, type="sc", linetype="solid", 
     alpha=0.5, cex=4)+
  ylim(-3,4)+xlim(0,60)+ggtitle(expression("Sr/"~delta^18~O~"cluster centroids"))+
  labs(x="transect cell", y="scaled value")+
  theme(axis.text = element_text(size = 25, color="black"),
        axis.title = element_text(size = 25),
        plot.title = element_text(size = 25, hjust = 0.5))
...
plot(Oclust_F$k_3, type="centroid", linetype="solid", cex=1)+
...
Salcentroid <- DBA(Bseries_Ores[1:10], trace = T, mv.ver = "by-variable")
matplot(do.call(cbind, Bseries_Ores[1:10]), type = "l")
points(Salcentroid)
...
write.table(fishassign_B, file = "Data/otoliths/Year_3_Bclusters.txt", row.names = F,
            dec = ".", sep = "\t")
write.table(fishassign_F, file = "Data/otoliths/Year_3_Fclusters.txt", row.names = F,
            dec = ".", sep = "\t")
...
fishassign_B <- read.delim("Data/otoliths/Year_3_Bclusters.txt", header = T,
                           stringsAsFactors = F, sep = "\t",dec=".")

fishassign_F <- read.delim("Data/otoliths/Year_3_Fclusters.txt", header = T,
                           stringsAsFactors = F, sep = "\t",dec=".")
...
#value + 2.518462
dataB$Sr <- dataB$Sr*1000
#This means that for interpretation, the actual value would be 4.1*10e3

dataF$d18O_res <- (dataF$d18O_res+sqrt((min(dataF$d18O_res))^2))
dataF$Sr <- dataF$Sr*1000
...
SeriesA$clust <- as.factor(SeriesA$clust)
elements <- mvabund(SeriesA[,3:8])

boxplot(elements)
meanvar.plot(elements)
elements_only <- elements
plot(elements~SeriesA$clust)


Pikedist <- vegdist(elements, method = "euclidean")
Disp <- betadisper(Pikedist, SeriesA$clust)
anova(Disp)

Per <- adonis2(elements~SeriesA$clust, method = "euclidean", permutations = 9999)
Per
```

```{r reclassification, include=FALSE}
rm(list = ls())
#read in data
load("Data/Otoliths/Adults_element_transects.RData")
...
set.seed(123)
sample <- sample(c(TRUE, FALSE), nrow(SeriesA), 
                 replace = T, prob = c(0.6,0.4))
train <- SeriesA[sample,]
test <- SeriesA[!sample,]
...
lda.m2 <- lda(clust~Sr_orig+Sr_juv+Sr_adult+O_orig+O_juv+O_adult, 
              data = SeriesA, CV=TRUE)

SeriesA$jacknife <- lda.m2$class
SeriesA %>% summarise(lda.error=mean(clust != jacknife))

plot(lda.m1)

df.pred <- data.frame(predict(lda.m1, newdata=test))
lda.cm <- table(test$clust, df.pred$class)

list(LDA_model = lda.cm %>% prop.table()%>%round(3))

test$lda.pred <- df.pred$class

test %>% summarise(lda.error=mean(clust != lda.pred))
...
fishassign_B <- read.delim(file = "Data/otoliths/Year_3_Bclusters.txt", 
                           header = T, stringsAsFactors = F, dec = ".", sep = "\t")
fishassign_F <- read.delim(file = "Data/otoliths/Year_3_Fclusters.txt", 
                           header = T, stringsAsFactors = F, dec = ".", sep = "\t")
fishassign_B$mclust <- factor(rep(1,50), levels = 1, labels = "Marine")
fishassign_F$mclust <- factor(rep(2,12), levels = 2, labels = "FW")

allpike <- rbind(fishassign_B[,c(1:3,5,6)], fishassign_F)

#join increment data with clustering results
pikedat <- fishassign_B%>%
  transmute(ID=ID,
            capt=capt,
            area=area,
            clust=case_when(clust_O %in% "High saline|const temp"~"Hochsalin warm",
                            clust_O %in% "Int saline|Int temp"~"Mittelsalin kalt",
                            clust_O %in% "Low saline|shift temp"~"Niedigsalin Wechsel"),
            mclust=mclust)%>%
...
png(filename = "C:/Users/timor/Nextcloud2/Timo/BODDENHECHT/Geschwafel/Graphic repository/Growth_cluster_year3.jpg", width = 2400, height = 1600)
ggplot(pikedat[pikedat$i!="edge"&pikedat$i!="core",], 
       aes(i,rad, fill=clust))+
  scale_x_discrete(limits = c(as.character(c(1:3))),
                   breaks = c(as.character(c(1:3))))+
  labs(x="Alter (Jahre)", y="Otolitheninkrement (\U00B5m)", title = "",
       fill="Ökotyp")+
  scale_fill_manual(breaks = c("Niedigsalin Wechsel","Mittelsalin kalt",
                             "Hochsalin warm"), values = mypallette)+
...
Salm <- lm(Salinity~Sr, data = fish_sal_sr)
summary(Salm)

a <- Salm$coefficients[[1]]
b <- Salm$coefficients[[2]]

newdata <- data.frame(Sr = seq(0.001,0.006, 0.0005))

newdata$salinity2 <- predict(Salm, newdata)
...
log_posterior <- function(params, data){
  ...
  #liklihood
  logpr = sum(dnorm(y, mu, s, log = TRUE))
  
  #priors
  logpr=logpr+dnorm(a,0,10,log=TRUE)+dnorm(b,0,5,log=TRUE)+dexp(s,0.2,log=TRUE)
  logpr
}

data <- with(fish_sal_sr, data.frame(x=Sr*1000, y=Salinity))

param_init <- c(a=0, b=0, s=1)

fit <- optim(param_init, log_posterior, data=data, method = "Nelder-Mead",
...
newdata$Sal3 <- a+b*(newdata$Sr*1000)
...
ggplot(fish_sal_sr, aes(Sr, Salinity, color=capt))+geom_point()+theme_minimal()
```
...
```{r pike database retrieval}
pikeDB <- read.delim("Data/1Pike_Latest_Version.txt", header = T, sep = ";",
                     dec = ".", stringsAsFactors = F)

Pike <- read.delim("Data/SIMS-pike_2022_id.txt", sep = ";")
Pikelist <- data.frame(Fish_ID=unique(Pike$ID))

SIMS_pike2022 <- pikeDB %>%
  right_join(Pikelist, by = "Fish_ID")%>%
  transmute(ID=Fish_ID,
            Scales_magnification=NA,
            sex=Sex,
            Weight=Weight,
            TL=Total_Length_mm,
            date=Date,
            ...
            Conductivity=Conductivity_myS_cm,
            Salinity=Salinity_PSU,
            area=Waterbody)

write.csv2(SIMS_pike2022, file = "Data/SIMS-pike_2022_data.csv")
write.table(SIMS_pike2022, file = "Data/SIMS-pike_2022_data.txt", row.names = F,
            sep = "\t", dec = ".")
write.table(Pikelist, file = "Data/Otolith-pike_Sehrow_Barthe_Spring-2022_TR.txt",
            row.names = F, sep = "\t", dec = ".")

pike_genetic <- read.csv2("Data/Genetics_Overview.csv")

Genetics_done <- pike_genetic %>%
  right_join(Pikelist, by="Fish_ID")

pikeDB[pikeDB$Fish_ID=="BH-93066",]$Waterbody
```
